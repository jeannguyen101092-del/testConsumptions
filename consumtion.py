...
# 1. HIỂN THỊ ĐỐI SOÁT HÌNH ẢNH HAI BÊN - GIẢI MÃ NHỊ PHÂN ĐỒNG BỘ NGUYÊN VĂN TÊN FILE CÓ DẤU GẠCH
...
with img_col2:
    if matched_techpack:
        ...
        # ⚡ SỬA LỖI MẤU CHỐT: Gọi chính xác tên mã nguyên bản (Giữ nguyên dấu gạch ngang R09-490416) để trùng khớp 100% với file trên Storage
        secure_img_url = f"{SB_URL.rstrip('/')}/storage/v1/object/public/kho_anh/{target_style_name}.jpg"
        
        try:
            # Gửi lệnh requests kèm apikey bảo mật bốc thẳng file nhị phân ảnh từ Supabase
            ...
            if img_response.status_code == 200 and len(img_response.content) > 100:
                ...
                # Luồng dự phòng lùi về gọi link xóa dấu gạch nếu có file khác viết liền
                style_clean_id = re.sub(r'[^a-zA-Z0-9]', '', str(target_style_name))
                fallback_url = f"{SB_URL.rstrip('/')}/storage/v1/object/public/kho_anh/{style_clean_id}.jpg"
                ...
                if img_res_fb.status_code == 200 and len(img_res_fb.content) > 100:
                    ...
                    st.image("https://unsplash.com", caption=f"⚠️ File ảnh vật lý {target_style_name}.jpg chưa đồng bộ hoặc rỗng.", use_container_width=True)
        except Exception:
            st.image("https://unsplash.com", caption=f"⚠️ Gặp sự cố kết nối luồng ảnh vật lý của mã {target_style_name}.", use_container_width=True)
    else:
