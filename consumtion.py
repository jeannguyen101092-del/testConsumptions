...
# ==============================================================================
# PHÂN HỆ CHỨC NĂNG BỔ SUNG: QUẢN LÝ KỸ THUẬT & ĐỊNH MỨC SẢN XUẤT PPJ
# ==============================================================================

def display_ts_comparison(df_ts_compare, title_text="📊 ĐỐI CHIẾU THÔNG SỐ KỸ THUẬT (T/S)"):
    """Chức năng: So sánh thông số kỹ thuật tinh gọn, làm nổi bật điểm lệch biên độ"""
    st.markdown(f"<div class='component-title-box'>{title_text}</div>", unsafe_allow_html=True)
    st.markdown("""
            ...
            <div class='data-table-container'>
                <table class='industrial-table'>
                    <thead>
                        <tr>
                            <th>Vị Trí Đo (POM)</th>
                            <th>Thông Số Gốc (Target)</th>
                            <th>Thông Số Thực Tế (Actual)</th>
                            <th>Chênh Lệch (+/-)</th>
    ...
    for _, row in df_ts_compare.iterrows():
        diff = row['actual'] - row['target']
        if abs(diff) > row.get('tolerance', 0.5):
            status_tag = f"<span style='color:#DC2626; font-weight:700;'>❌ Vượt biên ({diff:+.1f})</span>"
            row_bg = "background-color: #FEF2F2;"
        else:
            status_tag = f"<span style='color:#16A34A; font-weight:700;'>✅ Đạt chuẩn</span>"
            row_bg = ""
            
        st.markdown(f"""
            <tr style='{row_bg}'>
                <td><b>{row['pom_name']}</b></td>
                <td>{row['target']} cm</td>
                <td>{row['actual']} cm</td>
                <td>{diff:+.1f} cm</td>
                <td>{status_tag}</td>
...
def display_bom_allowance(bom_data):
    """Chức năng: Xuất kết quả định mức Nguyên Phụ Liệu (BOM) chuẩn công nghiệp"""
    st.markdown("<div class='component-title-box'>🧵 BẢNG TỔNG HỢP ĐỊNH MỨC NGUYÊN PHỤ LIỆU (BOM)</div>", unsafe_allow_html=True)
    st.markdown("<div class='card-container'><div class='data-table-container'>", unsafe_allow_html=True)
    st.markdown("""
        <table class='industrial-table'>
            <thead>
                <tr>
    ...
    for item in bom_data:
        st.markdown(f"""
            <tr>
                ...
                <td><span style='color:#2563EB; font-weight:700;'>{item['consumption']}</span> {item['unit']}/sp</td>
                <td>{item['width_inch']}</td>
                <td>{item['shrinkage']}</td>
                <td><span style='color:#1E3A8A; font-weight:600;'>{item['efficiency']}%</span></td>
                <td>{item['note']}</td>
            </tr>
        """, unsafe_allow_html=True)
    st.markdown("</tbody></table></div></div>", unsafe_allow_html=True)
...
def display_production_summary_table(summary_data):
    """Chức năng: Xuất bảng tính tác nghiệp tổng phục vụ phân xưởng cắt"""
    st.markdown("<div class='component-title-box'>📋 BẢNG TÍNH TÁC NGHIỆP TỔNG (PRODUCTION OPERATIONS SHEET)</div>", unsafe_allow_html=True)
    st.markdown("<div class='card-container'><div class='data-table-container'>", unsafe_allow_html=True)
    st.markdown("""
        <table class='industrial-table'>
            <thead>
                <tr>
                    ...
                    <th>Tỷ Lệ Sơ Đồ (Ratio)</th>
                    <th>Số Bàn Cắt Dự Kiến</th>
                    <th>Tổng Vải Tiêu Hao (Yds)</th>
                </tr>
            </thead>
            <tbody>
    """, unsafe_allow_html=True)
    ...
    for row in summary_data:
        st.markdown(f"""
            <tr>
                <td><b>{row['color_name']}</b></td>
                <td><b>{row['size']}</b></td>
                <td>{row['po_qty']:,} pcs</td>
                <td>{row['marker_ratio']}</td>
                <td>{row['planned_lays']} Bàn</td>
                <td><span style='color:#1E3A8A; font-weight:700;'>{row['total_fabric_req']:,} Yds</span></td>
            </tr>
        """, unsafe_allow_html=True)
    st.markdown("</tbody></table></div></div>", unsafe_allow_html=True)


def display_technical_comments(comments):
    """Chức năng: Đưa ra nhận xét và cảnh báo rủi ro sản xuất"""
    st.markdown("<div class='component-title-box'>⚠️ CẢNH BÁO RỦI RO KỸ THUẬT & SẢN XUẤT (COMMENTS)</div>", unsafe_allow_html=True)
    st.markdown("<div class='card-container'>", unsafe_allow_html=True)
    for risk in comments:
        st.markdown(f"""
            <div style='background-color: #FFF5F5; border-left: 5px solid #EF4444; padding: 14px 18px; border-radius: 6px; margin-bottom: 12px; color: #991B1B; font-size:13.5px;'>
                📌 <b>{risk['title']}:</b> {risk['desc']}
            </div>
        """, unsafe_allow_html=True)
...
# ==============================================================================
# KHU VỰC ĐIỀU HƯỚNG SIDEBAR & HIỂN THỊ MAIN PAGE
# ==============================================================================

st.sidebar.markdown("""
    <div class='sidebar-brand-container'>
        <div class='sidebar-brand-title'>PPJ GROUP</div>
        <div class='sidebar-brand-subtitle'>Techpack AI System</div>
    </div>
""", unsafe_allow_html=True)

# Menu chuyển đổi giữa các bài toán nghiệp vụ
app_mode = st.sidebar.selectbox(
    "📂 LỰA CHỌN PHÂN HỆ NGHIỆP VỤ:",
    [
        "Phân hệ chính: Số hóa Techpack",
        "Bài toán 1: Đối chiếu T/S & Định mức BOM nhanh",
        "Bài toán 2: Tính định mức đặt hàng theo Rập & Tài liệu mới"
    ]
)

# --- XỬ LÝ HIỂN THỊ THEO TỪNG BÀI TOÁN TỪNG Ý MỘT ---

if app_mode == "Bài toán 1: Đối chiếu T/S & Định mức BOM nhanh":
    # Ý 1: So sánh T/S gọn đẹp
    ts_data_1 = pd.DataFrame([
        {"pom_name": "Across Chest (1\" below armhole)", "target": 52.0, "actual": 52.6, "tolerance": 0.5},
        {"pom_name": "Total Body Length (HPS)", "target": 74.0, "actual": 74.2, "tolerance": 0.5},
        {"pom_name": "Sleeve Length (From back center)", "target": 86.0, "actual": 85.3, "tolerance": 0.5}
    ])
    display_ts_comparison(ts_data_1, title_text="📊 SO SÁNH THÔNG SỐ KỸ THUẬT (T/S THỰC TẾ vs TECHPACK GỐC)")
    
    # Ý 2: Kết quả Định mức theo BOM đầy đủ thông số
    bom_data_1 = [
        {"material_name": "Vải chính (100% Cotton Denim 12oz)", "consumption": 1.45, "unit": "Yds", "width_inch": "58\"", "shrinkage": "W: 3.2% | F: 1.5%", "efficiency": 88.5, "note": "Vải chính cho thân trước và thân sau"},
        {"material_name": "Chỉ may (Coats Phong Phú 40/2)", "consumption": 120.0, "unit": "Meters", "width_inch": "N/A", "shrinkage": "0%", "efficiency": 95.0, "note": "Chỉ may trần nổi túi sau"}
    ]
    display_bom_allowance(bom_data_1)
    
    # Ý 3: Comment rủi ro
    comments_1 = [
        {"title": "Độ co vải dọc (Warp Shrinkage) cao", "desc": "Độ co dọc đạt 3.2%, dễ dẫn đến hụt chiều dài quần sau khi giặt Wash. Cần cộng hao hụt vào Rập sản xuất."},
        {"title": "Hiệu suất sơ đồ size lớn thấp", "desc": "Khổ vải 58 inch gây tổn hao đầu tấm khi đi sơ đồ phối hợp size XL. Khuyến nghị chạy sơ đồ cụm riêng lẻ."}
    ]
    display_technical_comments(comments_1)

elif app_mode == "Bài toán 2: Tính định mức đặt hàng theo Rập & Tài liệu mới":
    # Ý 1: So sánh T/S với báo giá cũ hoặc mã cũ
    ts_data_2 = pd.DataFrame([
        {"pom_name": "Waist Relaxed (Vòng cạp êm)", "target": 38.0, "actual": 40.0, "tolerance": 1.0},
        {"pom_name": "Hip Width (Vòng mông)", "target": 48.0, "actual": 51.5, "tolerance": 1.0}
    ])
    display_ts_comparison(ts_data_2, title_text="🔄 ĐỐI CHIẾU THÔNG SỐ T/S HIỆN TẠI VỚI BÁO GIÁ SƠ BỘ / MÃ HÀNG CŨ")
    
    # Ý 2: Kết quả định mức BOM đặt hàng
    bom_data_2 = [
        {"material_name": "Vải chính (Kaki Stretch Twill)", "consumption": 1.22, "unit": "Yds", "width_inch": "56\"", "shrinkage": "W: 2.0% | F: 4.5%", "efficiency": 91.2, "note": "Tính toán dựa trên diện tích nhảy mẫu Rập"},
        {"material_name": "Mếch lót cạp (Interlining)", "consumption": 0.15, "unit": "Meters", "width_inch": "44\"", "shrinkage": "0%", "efficiency": 98.0, "note": "Mếch giấy hột"}
    ]
    display_bom_allowance(bom_data_2)
    
    # Ý 3: Bảng tính tác nghiệp tổng
    summary_data_2 = [
        {"color_name": "Dark Navy", "size": "M", "po_qty": 2500, "marker_ratio": "1M + 2L", "planned_lays": 5, "total_fabric_req": 3050},
        {"color_name": "Dark Navy", "size": "L", "po_qty": 3000, "marker_ratio": "1M + 2L", "planned_lays": 6, "total_fabric_req": 3660}
    ]
    display_production_summary_table(summary_data_2)
    
    # Ý 4: Comment rủi ro đặt hàng
    comments_2 = [
        {"title": "Độ co sợi ngang (Weft) biến động mạnh", "desc": "Vải co ngang 4.5%, rủi ro méo ống hoặc vặn ống cao nếu công nhân rải vải không thả chùng lực căng đầu cây."},
        {"title": "Tỷ lệ hao hụt bàn cắt tăng", "desc": "Số lượng cỡ vóc lệch nhau nhiều giữa các PO. Khuyến nghị phòng Cung ứng đặt dư dôi thêm 2% vải để bù đầu tấm cắt lỗi."}
    ]
    display_technical_comments(comments_2)
