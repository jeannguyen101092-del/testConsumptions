...
# -----------------------------------------------------------------------------
    ...
    if file1 and file2:
        # Tối ưu hóa việc xử lý file bằng vòng lặp ngắn gọn
        for f in [file1, file2]:
            if f.name not in st.session_state["processed_styles"]:
                res = process_single_pdf_batch(f.getvalue(), f.name)
                if res["success"]: 
                    st.session_state["processed_styles"][f.name] = res["data"]
        ...
        if d1 and d2:
            ...
            for pom in sorted(all_poms):
                ...
                # Tối ưu hóa style hiển thị Delta trực quan hơn
                if delta > 0:
            ...
            # --- KHỐI XỬ LÝ VÀ ĐỊNH DẠNG EXCEL ĐÃ SỬA LỖI & LÀM ĐẸP ---
            ...
            with pd.ExcelWriter(towrite, engine='xlsxwriter') as writer: 
                ...
                # Định dạng bảng chuẩn Techpack chuyên nghiệp
                ...
                # Ghi header và căn chỉnh độ rộng cột tự động
                for col_num, column_title in enumerate(df_compare.columns):
                    worksheet.write(0, col_num, column_title, header_format)
                    max_len = max(df_compare[column_title].astype(str).map(len).max(), len(column_title)) + 4
                    worksheet.set_column(col_num, col_num, max_len)
                
                # Căn lề dòng dữ liệu (Cột đầu canh trái, các cột thông số canh giữa)
                for row_idx in range(len(df_compare)):
                    worksheet.write(row_idx + 1, 0, df_compare.iloc[row_idx, 0], left_format)
                    for col_idx in range(1, len(df_compare.columns)):
                        worksheet.write(row_idx + 1, col_idx, df_compare.iloc[row_idx, col_idx], center_format)
            
            towrite.seek(0)
            
            # Thêm nút tải file Excel với UI Streamlit chính quy
            st.download_button(
